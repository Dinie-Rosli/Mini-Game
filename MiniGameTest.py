...
if (Location2 == "right") or (Location2 == "2") or (Location2 == "right") or (Location2 == "rights"):
    ...
    if Health>0 and EnragedstumpHealth<=0:        
        print("\nYour remaining health is " +str(Health))
## Passed the 2nd engagement
pots = 1+pots; print("\nCongratulations, you have killed the enemy. It dropped a health potion! You now have "+ str(pots)+" left. Before you continue on..."); time.sleep(2.0);
print("Your proficiency with the " +str(Weapon)+ " has increased. You now deal more damage with the weapon"); Health = 40 + 60; time.sleep(1)
print("You have leveled up! Your health has increased to "+str(Health)+". You are back at full health and ready to go.\n"); time.sleep(1)
print("You've come across a long, brick bridge... You sense a great danger lies ahead, one that will pose a real threat. \nAs a hero, you don't back down and decide to cross the bridge to face the monster.\n"); time.sleep(2)
## Boss engagement
TrollHealth = 200;time.sleep(1.5); print("A sudden roar came from beneath the bridge 'Fresh MEAT!!!!' A Troll has appeared before you. They are infamous for their endurance and strength. Prepare!");pausecont()
while (TrollHealth>0) and (Health>0):
    print("Your Health is " + str(Health))
    print("Enemy health is: " + str(TrollHealth) + "\n")
    TrollDamage = random.sample(range(5,10),5).pop()+random.sample(range(4,9),5).pop()+random.sample(range(3,6),3).pop()
    if (Weapon == "sword"):
        dmg = random.sample(range(15,25),10).pop()
        if (Health<=30) and (pots>0):
            potion = input("\nDo you want to use a health potion?" +" You have " + str(pots) +" potion left. Y/N? \n").lower()
            if potion == "y" or potion == "yes":
                Health += 20
                ...
                print("Your health went up by 20! Your new health is: " + str(Health) + "\n");time.sleep(1)
                Health -=TrollDamage
                print("The enemy dealt " +str(TrollDamage) + " to you\n");time.sleep(1)
                continue
            else:
                pass
        TrollHealth -=dmg; pause()
        print("\n" + name + " dealt " + str(dmg) + " dmg to the enemy");time.sleep(1)
        if TrollHealth<=0:
            print("Troll is dead")
            break
        ...
        dmg = random.sample(range(22,32),10).pop()
        if (Health<=30) and (pots>0):
            potion = input("\nDo you want to use a health potion?" +" You have " + str(pots) +" potion left. Y/N? \n").lower()
            if potion == "y" or potion == "yes":
                Health += 20
        ...
        dmg = (random.sample(range(10,16),6).pop()+(random.sample(range(8,14),6).pop()))
        if (Health<=30) and (pots>0):
            potion = input("\nDo you want to use a health potion?" +" You have " + str(pots) +" potion left. Y/N? \n").lower()
            if potion == "y" or potion == "yes":
                Health += 20
        ...
        dmg = (random.sample(range(4,16),12).pop())+(random.sample(range(4,12),8).pop())+(random.sample(range(4,8),4).pop())
        if (Health<=30) and (pots>0):
            potion = input("\nDo you want to use a health potion?" +" You have " + str(pots) +" potion left. Y/N? \n").lower()
            if potion == "y" or potion == "yes":
                Health += 20
